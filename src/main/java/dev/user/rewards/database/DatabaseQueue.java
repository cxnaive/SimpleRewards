...
public class DatabaseQueue {
    ...
    public DatabaseQueue(SimpleRewardsPlugin plugin) {
        this.plugin = plugin;
        this.taskQueue = new LinkedBlockingQueue<>();
        this.executor = Executors.newSingleThreadExecutor(r -> {
            Thread t = new Thread(r, "SimpleRewards-DB-Queue");
            t.setDaemon(true);
            return t;
        });
     ...
     * @param callback 成功回调（可选，在队列线程中调用）
     * @param errorCallback 错误回调（可选，在队列线程中调用）
     */
